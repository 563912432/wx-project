<template>
  <div class="exam">
    <div class="head">
      ...
      <div class="title">
        <span>正在考试（{{exam_index ? exam_index+1 : 1}}/{{exam_length}}） {{curr.type_name ? curr.type_name.name : ''}}</span>
        <v-djs :time="+paperTime"></v-djs>
      </div>
    ...
    <div class="wrapper" v-if="exam">
      ...
      <div v-else-if="+curr.type === typeList.material">
        <material :exam="curr" :index="subExamInfo.index" :len="subExamInfo.len" :orientation="orientation"></material>
      </div>
      <!--计算分析题中级职称-->
    ...
    <card :exam="exam" :exam_index="exam_index" popup-transition="popup-fade"></card>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      goBack () {
        if (this.$store.state.paperInfo.type === '2') {
          ...
          callback: function (res) {
            ...
            switch (res.status) {
              case 1:
                ...
                that.paperTime = that.$store.state.paperInfo.time
                break
              case -1:
                instance = Toast({
                  message: res.info,
      ...
      doWell () {
        this.$router.push({path: '/result/' + this.$route.params.pid})
      },
      card () {
      ...
      saveProgress () {
        let that = this
        MessageBox.prompt('请输入进度名称', {inputType: 'text'}).then(function ({value, action}) {
          if (value) {
            if (/[\w\u4E00-\u9FA5\uF900-\uFA2D]/g.exec(value)) {
              ...
              let data = {
                param: {
                  random: signatureObj.random,
                  signature: signatureObj.signature,
                  cid: that.$route.params.tid,
                  pid: that.$route.params.pid,
                  uid: that.$store.state.userInfo.id,
                  tag: value,
                  remainder_time: that.$store.state.examState.leftTime,
